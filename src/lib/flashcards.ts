...
/**
 * Lista os assuntos distintos dos flashcards de uma disciplina —
 * usado no filtro de assunto da central de Jogos.
 */
export function getSubjectsForDiscipline(
  ...
  const subjects = new Set(
    flashcards.filter((f) => f.disciplineSlug === disciplineSlug).map((f) => f.subject)
  );
